...
def main():
    ...
    im = Image(Point(300,300),"s1.gif") #ace of spades in this case
    im.draw(win)
    #NOTE: if this program is not *inside* the playingcards folder but
    #instead *in the same folder as* the playingcards folder (i.e.
    #"next to" the playingcards folder), you can change the
    #image file name to "playingcards/s1.gif" to access the image.
    #Doing this is preferable because it keeps the image files organized
    ...
    im = Image(Point(350, 250), suit + str(rank) + ".gif")
    im.draw(win)
